...
export class Contents extends MyDisplay {
  ...
  constructor(opt:any) {
    ...
    particleOptions = Common.extend({ inertia: Infinity, friction: 0.2, collisionFilter: { group: group }, render: { visible: false }}, particleOptions);

    let constraintOptions:any = {};
    constraintOptions = Common.extend({ stiffness: 0.8, render: { type: 'line', anchors: false, visible: false } }, constraintOptions);

    let columns = Conf.instance.COLUMNS;
    let rows = Conf.instance.ROWS;
    const areaW = sw * Func.instance.val(0.8, 0.25);
    const areaH = areaW;
    ...
    // マウス
    const mouseSize =  Math.max(sw, sh) * 0.01
